...
from random import randint, choices, randrange, choice
...
class MoneyModel(Model):
    def __init__(self, kelpAmnt, seaCowAmnt, width, height):
        ...
        for i in range(kelpAmnt):
            uniqueIDcounter += 1
            a = kelpAgent(uniqueIDcounter, self)
            self.schedule.add(a)
        ...
        for j in range(seaCowAmnt):
            uniqueIDcounter += 1
            a = seaCowAgent(uniqueIDcounter, self)
            self.schedule.add(a)
            # Add the agent to a random grid cell
            x = randrange(self.grid.width)
            y = randrange(self.grid.height)
...
class seaCowAgent(Agent):
    ...
    def pathfinding(self):
        ...
        self.food = self.findFood()
        if type(self.food) != tuple:
            self.foodpos = self.food.pos
        else:
            self.foodpos = self.food

        #if there is no food, return
        if self.food == "no food":
            return

        #if the selected kelp has no food, choose a different one
        if type(self.food) != tuple:
            if self.food.availableFood < 1:
                self.food = self.findFood()

        #get vars for the actual A* movement
        foodx, foody = self.foodpos
        ...
        for neighbor in possiblePlaces:
            #if it can eat, do so
            if self.foodpos == neighbor and type(self.food) != tuple:
                self.eat()
    ...
    def eat(self):
        self.currentFood += self.food.availableFood
        self.food.availableFood = 0
