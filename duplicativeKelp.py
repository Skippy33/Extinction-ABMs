...
from mesa.space import Grid, GridContent, MultiGrid
...
class MoneyModel(Model):
    ...
    def step(self):
        ...
        for x in killList:
            self.model.grid.remove_agent(x)
...
class kelpAgent(Agent):
    ...
    def grow(self):
        ...
        a = kelpAgent(self.model.uniqueIDcounter, self.model)
        #add to scheduler and creat
        self.model.schedule.add(a)
        possiblePlaces = self.model.grid.get_neighborhood(
            self.pos,
            moore=True,
            include_center=False
        )
        shuffle(possiblePlaces)
        for place in possiblePlaces:
            if self.model.grid.is_cell_empty(place) and not self.model.grid.out_of_bounds(place) and place:
                self.model.grid.place_agent(a, place)
                return
